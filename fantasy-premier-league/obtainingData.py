...
def get_results_data(season, path, premier_league=True):
    ...
    if response.status_code != 200:
        raise Exception("Response was code " + str(response.status_code))
    response_text = (response.text).encode()
    response_path = path + 'results.csv'
    with open(response_path, 'wb') as f:
        f.write(response_text)

    headers_of_interest = ['Date', 'HomeTeam', 'AwayTeam', 'FTHG', 'FTAG']
    df = pandas.read_csv(response_path)
    df = df[headers_of_interest]
    df.to_csv(response_path, index=None, header=True)
